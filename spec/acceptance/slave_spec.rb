...
describe 'puppet-jenkins slave module', :if => ['debian', 'ubuntu'].include?(os[:family]) do
  ...
  it 'debug' do
    ...
    hdr = "Basic #{pw}"
    r = command("/usr/bin/curl --verbose --location \"http://`hostname -f`/pluginManager/api/json?depth=2\" -H 'Authorization: #{hdr}'")
    puts r.stdout
    puts r.stderr
  end
  it 'debug2' do
    r = command("/usr/bin/openssl s_client -connect `hostname -f`:443")
    puts r.stdout
    puts r.stderr
  end
  it 'debug3' do
    r = command("/bin/cat /etc/apache2/sites-enabled/*")
    puts r.stdout
    puts r.stderr
  end
end
